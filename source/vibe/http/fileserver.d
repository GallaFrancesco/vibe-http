...
*/
HTTPServerRequestDelegateS serveStaticFiles(PATH)(PATH local_path)
	if(is(PATH == NativePath) || is(PATH == string))
{
	HTTPFileServerSettings settings;
	return serveStaticFiles(local_path, settings);
}
...
unittest {
	...
	void setupRoutes()
	{
		...
		HTTPFileServerSettings fsettings;
...
/**
	...
	Params:
		...
		settings = Optional settings object enabling customization of how the
			file gets served.
*/
void sendFile(scope HTTPServerRequest req, scope HTTPServerResponse res, NativePath path, HTTPFileServerSettings settings)
{
...
private void sendFileImpl(scope HTTPServerRequest req, scope HTTPServerResponse res, NativePath path, HTTPFileServerSettings settings)
{
